...
import { CAT_META, SHIFT_LABELS, generateAssignments } from './utils/schedule'
...
export default function App() {
  ...
  // ── On mount: restore admin session + load latest schedule ──
  useEffect(() => {
  ...
  function generate() {
    ...
    setAssignments(generateAssignments(names))
    setGenerated(true)
    setEditMode(false)
    setEditCount(0)
  }
  ...
  function cycleShift(personIdx, dayName) {
    const isWknd = WEEKENDS.includes(dayName)
    setAssignments(prev => prev.map((p, i) => {
      if (i !== personIdx) return p
      const cur = p.schedule[dayName]
      let next
      if (isWknd) {
        next = cur === 'weekend' ? 'off' : 'weekend'
      } else {
        const cycle = ['morning', 'evening', 'off']
        next = cycle[(cycle.indexOf(cur) + 1) % cycle.length]
      }
      return { ...p, schedule: { ...p.schedule, [dayName]: next } }
    }))
    setEditCount(c => c + 1)
  }
  ...
  const dayRows = rangedays.map(({ dateStr, dayName, isWeekend }) => ({
    ...
    dateStr,
    isWeekend,
    morning: assignments.filter(p => p.schedule[dayName] === 'morning').map(p => p.name),
    evening: assignments.filter(p => p.schedule[dayName] === 'evening').map(p => p.name),
    weekend: assignments.filter(p => p.schedule[dayName] === 'weekend').map(p => p.name),
    off:     assignments.filter(p => p.schedule[dayName] === 'off').map(p => p.name),
  }))

  const maxM = Math.max(3, ...dayRows.map(r => r.morning.length + r.weekend.length))
  const maxE = Math.max(2, ...dayRows.map(r => r.evening.length))
  ...
  return (
    <div style={{ minHeight:'100vh', background:'#f3f4f6', padding:'16px' }}>
      <div style={{ maxWidth:1040, margin:'0 auto' }}>
        ...
        <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:14 }}>
          ...
          <div style={{ display:'flex', gap:8, alignItems:'center' }} className="no-print">
            ...
            {adminMode ? (
              <button className="btn btn-outline btn-sm" onClick={signOut} style={{ color:'#dc2626', borderColor:'#dc2626' }}>
                🔓 Admin — Sign Out
              </button>
            ) : (
        ...
        {/* ── Category legend ── */}
        <div style={{ display:'grid', gridTemplateColumns:'repeat(4,1fr)', gap:10, marginBottom:14 }} className="no-print">
          ...
          <div style={{ background:'white', borderRadius:12, border:'1px solid #e5e7eb', padding:20, marginBottom:14, boxShadow:'0 1px 3px rgba(0,0,0,.05)' }} className="no-print">
            ...
            <div style={{ display:'flex', gap:16, flexWrap:'wrap' }}>
              ...
              <div style={{ display:'flex', flexDirection:'column', gap:10, minWidth:170 }}>
                <div>
                  <label style={{ display:'block', fontSize:13, fontWeight:600, color:'#374151', marginBottom:5 }}>Week Starting</label>
                ...
                <div>
                  <label style={{ display:'block', fontSize:13, fontWeight:600, color:'#374151', marginBottom:5 }}>Week Ending</label>
            ...
            <div style={{ display:'flex', gap:8, marginTop:14, flexWrap:'wrap', alignItems:'center' }}>
              ...
              {generated && (
                <button className="btn btn-outline" onClick={() => { setEditMode(e => !e); if (!editMode) setView('byPerson') }}
        ...
        {editMode && (
          <div style={{ background:'#fef3c7', border:'1px solid #f59e0b', borderRadius:10, padding:'10px 16px', marginBottom:12, display:'flex', alignItems:'center', gap:12 }} className="no-print">
            ...
            <p style={{ fontSize:13, color:'#78350f' }}>
              <strong>Edit Mode</strong> — click any cell to cycle Morning → Evening → Off (weekends: On Duty ↔ Off). {editCount > 0 && `${editCount} change${editCount !== 1 ? 's' : ''} made.`}
            </p>
        ...
        {/* ── No schedule state (team view, nothing published) ── */}
        ...
        {!loadingDB && generated && (
          <>
            ...
            {view === 'byPerson' && <PersonTable rangedays={rangedays} assignments={assignments} editMode={editMode} onCycleShift={cycleShift} />}
      ...
      {/* ── Modals ── */}
      {showPinModal  && <PinModal onSuccess={() => setAdminMode(true)} onClose={() => setShowPinModal(false)} />}
