import { CAT_META, SHIFT_LABELS } from '../utils/schedule'
import { formatDate } from '../utils/dates'

export default function PersonTable({ rangedays, assignments, editMode, onCycleShift }) {
  return (
      ...
      <table style={{ fontSize:12, width:'100%' }}>
        <thead>
          <tr style={{ background:'#f3f4f6' }}>
            <th style={{ padding:'10px 12px', textAlign:'left', fontWeight:700, color:'#374151', minWidth:100, whiteSpace:'nowrap' }}>Date</th>
            {assignments.map((p, i) => {
              const m = CAT_META[p.catKey]
              return (
                <th key={i} style={{ padding:'8px 6px', textAlign:'center', fontWeight:700, color:'#374151', minWidth:85 }}>
                  <div>{p.name}</div>
                  <span style={{ display:'inline-block', padding:'1px 6px', borderRadius:20, fontSize:10, fontWeight:700, background:m.tagBg, color:m.tagColor, marginTop:2 }}>
                    {p.catKey}
                  </span>
                </th>
              )
                ...
                {!showSep && (
                  <>
                    ...
                    {assignments.map((p, pi) => {
                      const sv = SHIFT_LABELS[p.schedule[dayName]]
                      return (
                        <td key={pi}
                          className={editMode ? 'edit-cell' : ''}
                          onClick={() => editMode && onCycleShift(pi, dayName)}
                          style={{ background:sv.bg, color:sv.color, padding:'6px 4px', textAlign:'center', cursor: editMode ? 'pointer' : 'default' }}>
